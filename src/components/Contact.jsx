...
// Stores the error *key*, not the translated text — the text is resolved
// against the current language at render time, otherwise a message shown
// before a language switch stays frozen in the old language.
function validate(form) {
  const errors = {}
  if (form.elements.name.validity.valueMissing) errors.name = 'nameError'
  if (form.elements.email.validity.valueMissing) errors.email = 'emailRequiredError'
  else if (form.elements.email.validity.typeMismatch) errors.email = 'emailInvalidError'
  if (form.elements.message.validity.valueMissing) errors.message = 'messageError'
  return errors
}
    ...
    <section id="contact" className="py-24 md:py-[130px]">
      <div className="wrap">
        <Reveal className="max-w-xl mb-12">
          ...
          <h2 className="text-3xl md:text-[40px]">{t.contact.heading}</h2>
          <p className="text-muted mt-3">{t.contact.intro}</p>
        </Reveal>
        ...
        <Reveal delay={0.1} className="card-chunky overflow-hidden grid grid-cols-1 md:grid-cols-[0.85fr_1.15fr]">
          ...
          <div className="relative bg-[var(--primary-fill)] text-white p-6 md:p-8 flex flex-col justify-center overflow-hidden">
            ...
            <div className="relative flex flex-col gap-5">
              <div>
                <h3 className="text-white text-xl md:text-2xl mb-1.5">{t.contact.getInTouch}</h3>
                <p className="text-white/75 text-sm">{t.contact.dmText}</p>
              </div>
